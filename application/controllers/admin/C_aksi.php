...
class C_aksi extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/M_aksi', 'aksi');
        $this->aksi->upk = $this->session->userdata('upk');
    }
    ...
    function data()
    {
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->aksi->count_all(),
            "recordsFiltered" => $this->aksi->count_filtered(),
            "data" => $data,
        );
    ...
    function update()
    {
        ...
        if ($this->aksi->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->aksi->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    {
        if ($action == 'on') {
            if ($this->aksi->update(['status' => '1'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->aksi->update(['status' => '0'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->aksi->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->aksi->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->aksi->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
