...
class C_status extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/M_status', 'status');
        $this->status->upk = $this->session->userdata('upk');
    }
    ...
    {
        $data = $this->req->all();
        if ($this->status->insert($data) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function update()
    {
        ...
        if ($this->status->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->status->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->status->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->status->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->status->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
