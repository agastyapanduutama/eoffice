...
class C_adminsurat extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/M_adminsurat', 'adminsurat');
    }
    ...
    public function list()
    {
        $data = array(
            ...
            'menu' => 'admin-persuratan',
            'script' => 'adminsurat',
            'konten' => 'admin/adminsurat/list'
        );
    ...
    function getJabatan()
    {
        echo json_encode($this->adminsurat->data_jabatan());
    }

    function data()
    {
        error_reporting(0);
        $list = $this->adminsurat->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->adminsurat->count_all(),
            "recordsFiltered" => $this->adminsurat->count_filtered(),
            "data" => $data,
        );
    ...
    {
        if ($action == 'on') {
            if ($this->adminsurat->update(['status' => '1'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->adminsurat->update(['status' => '0'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->adminsurat->update(['password' => $this->req->acak('123')], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
    ...
    function insert()
    {
        ...
        if ($this->adminsurat->insert($data) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function update()
    {
        ...
        if ($this->adminsurat->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->adminsurat->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->adminsurat->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->adminsurat->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->adminsurat->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
